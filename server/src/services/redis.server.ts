...
const MAX_VOTES_PER_IP = parseInt(process.env.MAX_VOTES_PER_IP ?? "10", 10);

const ipRateKey = (pollId: string, rawIp: string) =>
    `ratelimit:${pollId}:ip:${createHash("sha256").update(rawIp).digest("hex")}`;
...
export const createPoll = async (
    ...
    try {
        ...
        console.warn("[DB] Failed to persist poll:", dbErr);
    }
...
export const castVote = async (
    ...
    const rateKey = ipRateKey(pollId, clientIp);
    const remainingTtlSec = Math.ceil((expiresAt - Date.now()) / 1000);

    const [[incrErr, ipCount]] = (await redis
        .pipeline()
        .incr(rateKey)
        .exec()) as [[Error | null, number]];

    if (!incrErr && ipCount === 1) {
        await redis.expire(rateKey, remainingTtlSec);
    }

    if (!incrErr && ipCount > MAX_VOTES_PER_IP) {
        await redis.decr(rateKey);
        console.warn(`[rate-limit] IP exceeded limit on poll ${pollId}`);
    ...
    try {
        ...
        console.warn("[DB] Failed to persist vote:", dbErr);
    }
...
export const listActivePolls = async (): Promise<PollSummary[]> => {
    ...
    for (const key of keys) {
        ...
        const vKey = key.replace(":meta", ":votes");
        pipeline.hgetall(vKey);
    }
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        polls.push({ id, question: meta.question, totalVotes, expiresAt });
    }
